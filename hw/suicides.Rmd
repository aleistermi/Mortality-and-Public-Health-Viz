...
geom_line(aes(x=yearmonth, y=index, group = non_natural_causes), data = subset(deaths_base100, non_natural_causes!="War Operations"), colour = alpha("grey", 0.7)) +
  # colourise only the filtered data
  geom_line(aes(x=yearmonth, y=index, colour = ami_cols("green")), data = death_filtered) +
  # colourise only the filtered data
  geom_line(aes(x=yearmonth, y=index), data = homicides_df, alpha=.3)
...
total_rate_plot = ggplot(TOTAL_monthly_death_national, aes(x=yearmonth, y=death_rate)) +
 geom_line(color=ami_cols("green"), alpha=.9)  + 

  stat_smooth(alpha=.04,
    method = "loess", se = FALSE, color= ami_cols("orange"), size=.5, alpha=.25 ) +
    ylim(25, 40) +
...
Once we only consider only natural deaths, we observe a similar behavior as the total deaths. That is, there is a noticeable smooth jump starting in 2005 that persists  until 2017.
...
natural_death_plot = ggplot(subset(monthly_death_national,natural== 1 ), aes(x=yearmonth, y=death_rate)) +
  ...
  geom_line(color=ami_cols("green"), alpha=.9)  +
   labs(title = "1B. Natural Deaths Mortality Rate in Mexico 1998-2017",
       subtitle = "Once we exclude non-natural causes, the increase in mortality rates persist, suggesting an increase in mortality \ndue to public health issues",
       caption = "Source: INEGI and CONAPO", 
       x = "Year", y = "Monthly Rate (per 100k)")  +
  stat_smooth(
   method = "loess", se = FALSE, color= ami_cols("orange"), size=.5, alpha=0.25) +
  xlab("") + 
  theme_ami
natural_death_plot
...
Starting in 2000, there is an important reduction in the number of people who died in an accident. The peak in the rate of deaths caused by accidents occured in December 2000, and it has remained stable since 2010. One hypothesis that may explain this pattern is the adoption of more agressive anti-DIU policies in big cities in Mexico, which occured during the 2000's. These policies noticeably reduced the number of transit accidents and deaths in cities like Mexico City, Monterrey and Guadalajara. A further exploration of this topic has to disect the variation of accident-related deaths by state and city, and type of accident.
...
accidents_plot=ggplot(subset(monthly_death_national,non_natural_causes %in% c("Accident")), aes(x=yearmonth,y=death_rate))+
  ...
  ylab('Monthly Rate') +
  geom_line(color=ami_cols("green"), alpha=.9)  +
  labs(title = "1C. Dwindling accidents",
...
homicide_plot=ggplot(subset(monthly_death_national,non_natural_causes %in% c("Homicide")), aes(x=yearmonth, y=death_rate)) +
  ylim(0, 2.5) +
  ylab('Monthly Rate') +
  geom_line(color=ami_cols("green"), alpha=0.9)  +
  ylab('Monthly Rate') +
  geom_vline(xintercept =2006.917 , linetype="dotted", 
                color = ami_cols("orange"), size=.5)+ 
    geom_text(aes(x=2007.1, label="War on drugs starts", y=.20), colour=ami_cols("orange"), hjust = 0) +
labs(title = "1D. Homicides rate: an upward trend",
       subtitle = "After a substantive decrease between 2011-2015,\n homicides rate shows a sharp upward trend again",
       caption = "Source: INEGI and CONAPO", 
       x = "Year", y = "Monthly Rate (per 100k)")+
theme_ami
homicide_plot
...
### E. War operations related deaths

This classificationis recent, and shows an interesting pattern. We can distinguish a period between 2011 and 2015 in which both the month-to-month levels and variation  increased. During this period, the spikes are more frequent and of larger magnitude, suggesting a very intense and volatile behavior in this type of deaths. Then, after 2015, the spikes length become shorter. A reasonable assumption is that these deaths are related to the army forces that participated in different operations, mostly related to the fight against drug cartels. An interesting hypothesis to explore is the fact that this period of large variation also corresponds to the period of homicides decline, suggesting that a major involvement of the army forces may had a dissuasive effect on the probability of commiting homicides.
...
war_plot=ggplot(subset(monthly_death_national,non_natural_causes %in% c("War Operations") & ANIO_OCUR>2003), aes(x=yearmonth, y=death_rate)) +
  ...
  geom_line(color=ami_cols("green"), alpha=0.9)  +
  stat_smooth(
    color = ami_cols("orange"), alpha=.25,
  ...
  geom_text(aes(x=2012.417, label="Decrease in \n homicides rate", y=.0265), colour=ami_cols("orange")) +
    labs(title = "1E. War operations: the costs of decreasing the homicides rate?",
...
### F. Deaths with no classification
...
dn_plot=ggplot(subset(monthly_death_national,non_natural_causes %in% c("Don't know")), aes(x=yearmonth, y=death_rate)) +
 ...
 geom_text(aes(x=2012.8, label="Decrease in \n homicides", y=.18), colour=ami_cols("orange")) +
  geom_vline(xintercept =2010.417, linetype="dotted", 
   ...
   geom_vline(xintercept =2015.083, linetype="dotted", 
                color = ami_cols("orange"), size=.5)+
    labs(title = "1F. State's inability to produce vital stats",
...
suicide_plot=ggplot(subset(monthly_death_national,non_natural_causes %in% c("Suicide")), aes(x=yearmonth, y=death_rate)) +
  ...
  ylab('Monthly Rate') +
  geom_line(color=ami_cols("green"), alpha=0.9)  +
 labs(title = "1G. Suicides: Ramping up",
...
# male_mean_17<- mean(subset(suicides_by_sex, sex="Male", year_death==2017)$age_years, na.rm=TRUE)
# male_mean_08<- mean(subset(suicides_by_sex, sex="Male", year_death==2008)$age_years, na.rm=TRUE)
# male_mean_98<- mean(subset(suicides_by_sex, sex="Male", year_death==1998)$age_years, na.rm=TRUE)
# female_mean_17<- mean(subset(suicides_by_sex, sex="Female", year_death==2017)$age_years, na.rm=TRUE)
# female_mean_08<- mean(subset(suicides_by_sex, sex="Female", year_death==2008)$age_years, na.rm=TRUE)
# female_mean_98<- mean(subset(suicides_by_sex, sex="Female", year_death==1998)$age_years, na.rm=TRUE)
