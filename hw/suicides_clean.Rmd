...
geom_line(aes(x=yearmonth, y=index, group = non_natural_causes), data = subset(deaths_base100, non_natural_causes!="War Operations"), colour = alpha("grey", 0.4), size=.4) +
 ...
 scale_x_discrete(limit = c(1998, 2003, 2008, 2013,  2018)) +
  annotate("label", x = 2008.7, y = 190, label = "Homicides", color=ami_cols("blue"), family="Karla", size=4, alpha=.6) +
  ...
  # colour = ami_cols("green2"))+
      labs(title = "Change in Death Rate for Different Causes of Deaths in Mexico",
...
### Spatial Distribution of Suicides at the Municipal Level
...
municipal_suicides_plot<-mxmunicipio_choropleth(mun_shape)+ scale_fill_manual("Rate/100K", values=c('#d2e2f0', '#90b8da','#4d8dc3', '#2171b5'), na.value=ami_cols("dark grey"))  +
