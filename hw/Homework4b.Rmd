...
violines<-ggplot(suicides_by_sex, aes(y=age_years, x=sex, fill=sex)) +
 ...
 geom_violin(trim = FALSE, draw_quantiles = c(0.25, .5, 0.75), linetype = "dashed",show.legend = FALSE)  + 
# stat_summary(fun.y = "mean", fun.ymin = "mean", fun.ymax = "mean", 
#                  colour = "black", aes(shape="total"), geom="point",  shape=17, size=2, show.legend = FALSE) +
...
dat_text <- data.frame(
  label = c(33.8, 35.4, 35.2),
  year_death   = c(1998, 2008, 2017),
  sex     = c("Female", "Female", "Female"),
  age_years     = c(33.8, 35.4, 35.2)
)

violines<-violines + geom_text(
  data    = dat_text,
  mapping = aes(y = age_years, x = sex,  label = label), size=3)
violines
```
...
municipal_suicides_plot<-mxmunicipio_choropleth(mun_shape)+ scale_fill_brewer("Rate/100K", type = "qual", palette= ami_pal(palette="green_orange")(4), na.value=ami_cols("dark grey")) +
  labs(title = "20-years Annual Average Municipal Suicide Rate (1998-2017)",
       legend = "Rate (#/100K)", 
       caption = "Source: INEGI, CONAPO and Diego Valle's Blog (https://www.diegovalle.net) ") +
  annotate(geom = "text", x = -95, y = 31, label = "Mexico National Average: 5.2", size = 5, hjust = 0, family="Karla") + annotate(geom = "text", x = -95, y = 30, label = "World Average: 10.5", size = 5, hjust = 0, family="Karla") + annotate(geom = "text", x = -95, y = 29, label = "US Average: 13.7", size = 5, hjust = 0, family="Karla") +
  theme_amiplots
