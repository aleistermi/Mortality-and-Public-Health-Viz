...
theme_ami<-
  ...
  panel.grid.minor = element_line(size = 0.1, linetype = 'solid',
                                colour = "dark grey")) +
  theme(plot.title = element_text(size = 14, family = "Karla"))+
...
total_rate_plot = ggplot(TOTAL_monthly_death_national, aes(x=yearmonth, y=death_rate)) +
  geom_line(color=ami_cols("green"))  + geom_text()
  stat_smooth(alpha=.04,
    ...
    ylim(25, 40) +
      labs(title = "1A. Monthly Mortality Rate in Mexico 1998-2017",
       ...
       x = "Year", y = "Monthly Rate (per 100k)", family='Dosis-SemiBold')  +
  geom_vline(xintercept =2006.917 , linetype="dotted", 
                color = ami_cols("orange"), size=.5)+ 
    geom_text(aes(x=2007.1, label="War on drugs starts", y=28, family="Dosis"), colour=ami_cols("orange"), hjust = 0) 
 #theme_ami
```
```{r}
p1 = ggplot(NULL, aes(x = 1, y = 1)) + ylim(0.8, 1.2) +
    annotate("text", 1, 1.15, label = "Karla es esta que obno",
             family = "Karla", size = 5) +
    ...
    annotate("text", 1, 1, label = "HindSiliguri-Regular",
...
p2 = ggplot(NULL, aes(x = 1, y = 1)) + ylim(0.8, 1.2) +
    ...
    annotate("text", 1, .82, label = "Impact",
             family = "Impact", size = 5) +
  theme(axis.title = element_blank(),
          axis.ticks = element_blank(),
          axis.text  = element_blank())
# font_import(paths = here("fonts (1)")) 
#loadfonts() advent pro economica o karla
```
...
## Male Plot ####
p_male <- ggplot(test_suicides %>% filter(!is.na(day_of_suicide) & sex=='Male'), aes(hour_suicide, day_of_suicide,  fill=n)) +  
