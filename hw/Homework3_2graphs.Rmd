...
heat_map_suicide_male <- ggplot(test_suicides %>% filter(!is.na(day_of_suicide) & sex=='Male'), aes(hour_suicide, day_of_suicide,  fill=n)) +  
 geom_tile(aes(fill = n), color="white") +
  ...
  ggtitle("Suicides by time and weekday")+
  labs( subtitle = " Male suicides occur around midnight all weekdays, \nand mostly on weekends, whereas female suicides are more evenly distributed in the weekdays",
      caption="" ,x = "Time", y="") +
...
#scale_fill_gradient2(high = "darkred") + #scale_x_discrete(position = "top")+
  ...
  theme(  axis.text.x=element_text( size = 6.5, family="Karla", face = "plain"))
library(cowplot)
heatmaps<-plot_grid(heat_map_suicide_male, heat_map_suicide_female, ncol=1, align = "v")
heatmaps
## HOMICIDES########
