...
plot<- ggplot(subset(suicides_male, year_death<2017), aes(x = age_years, color=year_death)) +
  geom_density(alpha = .001)
plot2<- ggplot(suicides_male, aes(x = age_years, color=year_death)) +
years<-unique(suicides_male$year_death)
...
plot_ages <- ggplot(data = suicides_male,
            mapping = aes(x = age_years, y = factor(rev(year_death)),
                                     ordered = TRUE))+
  geom_density_ridges(alpha = 0.6, fill = "lightblue", scale = 1.5) +
    ...
    labs(x = "Age", y = NULL,
         title = "Age Distribution of\n Suicide Cases in Mexico 1998-2017") +
    theme_ridges() +
