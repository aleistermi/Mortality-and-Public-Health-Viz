...
labs(title = "1E. Homicides rate: an upward trend that doesn't give up",
...
causesplot<-ggplot(causes_normalized_age_male, aes(x=age_years, y=percent, fill=cause_of_death)) + 
  ...
  labs(title = "2. Principal Male Mortality Causes in Mexico 1998-2017",
       caption = "Source: National Institute of Statistics and Geography (INEGI)", 
       x = "Age", y = "Percent") +
       theme(plot.title = element_text(lineheight=.8, face="bold"))
...
suicides_ratechange_plot <- ggplot(subset(mean_suicide, pop2015.x<100000), aes(x=pop2015.x, y=rate_suic_15, color=state_name))+ geom_point(bins = 20, alpha=1/10)+
...
test_suicides <-
  ...
  tally()
# suicides_time <- ggplot(test %>% filter(!is.na(day_of_suicide)), aes(x=hour, y=day_of_suicide)) + 
#   geom_point() 
# + 
#   scale_y_datetime(date_labels = "%H:%M", date_breaks = "1 hour")
...
#(aes(fill = n),colour = "white") + scale_fill_gradient(low = "white",high = "darkblue")
#+ stat_density(aes(fill = stat(density)), geom = "raster", position = "identity") +

p <- ggplot(test_suicides %>% filter(!is.na(day_of_suicide)), aes(hour_suicide, day_of_suicide,  fill=n)) +  
...
scale_fill_gradient2(high = "darkred") + #scale_x_discrete(position = "top")+
  ...
  ggtitle("Suicide cases occur around midnight all weekdays, and mostly on weekends")+
  labs( #subtitle = "Average rate of growth 1999-2017",
      caption="Source: INEGI and CONAPO" ,x = "Time", y = "Day") 
p_female <- ggplot(test_suicides %>% filter(!is.na(day_of_suicide) & sex=="Female"), aes(hour_suicide, day_of_suicide,  fill=n)) +  
  # stat_density(aes(fill = stat(count)), geom = "raster", position = "identity") +
  # #geom_raster() +
  #geom_raster() 
 #stat_density(aes(fill = stat(density)), geom = "raster", position = "identity")
...
scale_fill_gradient2(high = "darkred") + #scale_x_discrete(position = "top")+
    scale_x_continuous(position = "top", breaks=test_suicides$hour_suicide) +
  ggtitle("Suicide cases by weekday and time of the day")+
  labs( #subtitle = "Average rate of growth 1999-2017",
      caption="Source: INEGI and CONAPO" ,x = "Time", y = "Day") 
...
daily_homicides$day_of_homicide <- factor(daily_homicides$day_of_homicide, levels = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"))
...
suicides_by_cohort<-
  suicides_by_cohort %>% mutate(age_cohort = case_when(age_years<15 ~ 'less_15',
                                          age_years>=15 & age_years<35 ~ 'bet_15_35',
                                          age_years>=35 & age_years<50 ~ 'bet_35_50',
                                          age_years>=50 & age_years<70 ~ 'bet_50_70',
                                          age_years>=70 ~ 'more_70'))

suicides_by_cohort_by_sex <-
  suicides_by_cohort %>% 
  group_by(age_cohort, sex, year_death) %>% 
  tally()
...
suicides_by_cohort<-
  suicides_by_cohort %>% mutate(pop_divide = case_when(ANIO_OCUR<2000 ~ pop_by_cohort$`1995`,
                                          ANIO_OCUR==2000 & ANIO_OCUR<=2004 ~ pop_by_cohort$`2000`,
                                          ANIO_OCUR>=2005 & ANIO_OCUR<= 2009 ~ pop_by_cohort$`2005`,
                                          ANIO_OCUR>=2010 & ANIO_OCUR<= 2014  ~ pop_by_cohort$`2010`, ANIO_OCUR>=2015 ~ pop_by_cohort$`2015`))
