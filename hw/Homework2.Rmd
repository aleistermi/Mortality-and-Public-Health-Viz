...
# suicides_by_cohort_total<-suicides_by_cohort_total  %>%
#   mutate(year_merge=as.numeric(as.character(suicides_by_cohort_total$year_merge)))
...
#   right_join(pop_cohort2, by=c("age_cohort","year_merge")) 
...
```{r}
cohortsplot=ggplot(final_suicides_by_cohort %>% filter(!is.na(age_cohort)), aes(x=year_death, y=rate_suicides_bycohort)) +
  geom_line() +
