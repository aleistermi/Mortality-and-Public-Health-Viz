...
```{r causes pressure, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
...
"ggspatial", "libwgeom", "sf", "rnaturalearth", "rnaturalearthdata", "ggplot2", "rgeos"))
...
state_suicide_rate_plot<- mxhexbin_choropleth(df_mx_state, num_colors = 1) +
  scale_fill_gradient("Rate",low=ami_cols("green"), high = ami_cols("orange")) +
   labs(title = "Average Suicide Rate 1998-2017",
       ...
       caption = "Source: INEGI, CONAPO and https://www.diegovalle.net/")+
...
load(here("Data/Shape_Mun/Municipios.shp"))

mun_shape <- st_read(here("Data/muni_2012gw/Muni_2012gw.shp"))
state_shape<- st_read(here("Data/state_boundaries/State.shp"))
state_shape2<- st_read(here("data/state_boundaries/dest_2015gw/dest_2015gw.shp"))
path = here("death_certificates.RData")
print(path)
data = load(path)
...
mun_shape<- mun_shape %>%
  mutate(rate_cut = cut(mun_shape$rate, c(0,16,50,100,356))) 
...
border<- geom_polygon(data = state_shape2, fill = NA, color = "red") + 
    coord_fixed(1.3)
...
load(here("Data/diego_valle/df_mxstate.RData"))
