...
A similar story can be told about suicides in Mexico. 

Compared to the attention that other mortality characteristics receive, this phenomenon has been absent from the public discussion and, to our knowledge, there are no comprehensive studies that account for its characteristics, not to mention its causes. This work focuses on analyzing suicides in Mexico.[^*]

[^*]: I thank Alex Engler, Cory Rand and Sinclair Target for their insightful comments to improve this work. The contents, design and mistakes are my own.


Figure 1 shows the trend in suicides in Mexico since 1998 (green line). To show how the increase in suicides compares to its level in January 1998 and to the growth of other mortality causes, all death rates were set equal to a value of 100 in January 1998. Then the value of this index reflects how much the value of a mortality rate changed as compared to the value of 100 in 1998, and how this growth compares to other mortality growth along the period 1998-2017.


The dramatic increase in homicides, which is arguably the most important policy challenge faced by Mexican authorities, has received special attention in the last 12 years. However, as we may find an exogeous cause that triggered the spiral of violence in the country (namely, the so called "war on drugs"), we don't find an exact date or cause that points the beginning of the sucides' upward trend. Nonetheless, we observe a sustained growth since 1998: the ascendant trend in suicides started in the late 1990's and has kept growing since. This growth has been only recently surpassed by the increase in  homicides (blue line), which rate has doubled since 1998, whereas in the case of suicides, the growth is about 50%.
...
Different analysis of suicides point out that there are important variations across population cohorts. In the case of Mexico, these differences are remarkable. The suicides rate shows the largest increase among the young population: for the 15-35 years old cohort, rate increased from around 6 cases per 100k inhabitants, to a little bit more than 8 in 2017; an increase of about 30%. This growth is also remarkable among the 35-50 cohort, which rose from around 5 to 7 cases per 100k. Among the 50-70 cohort we observe a relatively stable average (though it may seem to exist a seasonality pattern). Finally, in the elder cohort we see a sustained drop in suicides (from around 7 to around 5 cases per 100k).
...
As relevant as the variation among cohorts,  differences between sex are noticeable. From the total cases of suicides since 1998, 82% correspond to male suicides, and only 18% to female. The violin plot in Figure 3 analyzes the distribution of ages across males and females who commited suicide. The dotted lines correspond to the percentiles 25th, 50th, and 75th, and the number within the chart corresponds to the average age for each group. 

There are three important things to notice from the violinplot. First, women who committed suicide are on average younger than men by an approximate difference of 6 years (in 2017, 31.6 vs 36.1 years old). For all the period 1998-2017, the mean age for males is 36.1 years old, and females' is 30.5. Second, the average age of suicide has slightly increased in the last 20 years for both men and women. This increase seems to be concentrated in the age cohort between 25 and 50 years old. Third, the distribution of male age has become more evenly distributed, in the sense that the shape of the violin plot has become more cylindrical or thin; that is, there are more cases evenly distributed among the young male population (less than 50 years old). On the other hand, the distribution of female cases keeps a more clear "pear" shape.
...
geom_text(
  data    = dat_text2,
  mapping = aes(y = age_years, x = Sex,  label = label), size=3, family="Karla")+
  ggtitle("3. Age Distribution of Suicide Cases by Sex in 1998, 2008 and 2017")+
  labs( subtitle = "Females commit suicide younger. Among males, suicides are becoming more evenly distributed in ages 25-40's",
      caption="Source: INEGI \n Dotted lines correspond to the 25th, 50th and 75th percentiles" ,x = "Sex", y = "Years") +
  theme_ami
...
Map 1 shows the rate of suicides at the municipal level. The break values used to color the map are  the national Mexican average (5.2 cases per 100 thousand inhabitants); the world average (10.5), and the US average (13.7). 

This map shows clusters of municipalities with very high suicide rates in the north-east part of the country (Sonora and Chihuahua states); the north of Yucatan;  the north-east (states of Nuevo Leon and Tamaulipas) and the state of Oaxaca. In the former case, a lot of Oaxaca's municipalities have a very small population, which inflates the effect of an addional case of suicides, making the rates to look very high. In fact, Oaxaca state is not among the top states in terms of suicide rates. Chihuahua and Yucatan, on the other hand, are among the states with the larger rates of suicides. Explaining the causes of high rates are not the scope of this work, but a further exploration of the sociodemographic characteristics of the suicide population is presented later.
...
municipal_suicides_plot<-mxmunicipio_choropleth(mun_shape)+ scale_fill_manual("Rate/100K", values=c('#f5e5cf','#e7be88', '#dea459', '#d07e12'), na.value=ami_cols("dark grey"))  +
  ...
  annotate(geom = "text", x = -96, y = 31, label = "Mexico's Average: 5.2", size = 3.5, hjust = 0, family="Karla") + annotate(geom = "text", x = -96, y = 30, label = "World Average: 10.5", size = 3.5, hjust = 0, family="Karla") + annotate(geom = "text", x = -96, y = 29, label = "US Average: 13.7", size = 3.5, hjust = 0, family="Karla") +
  theme_amiplots
...
Maps 2 and 3 show the State's average age of suicide cases by sex. The breaks are 25% percentiles. Noticeable, the younger suicide cases occur at the center ( "GTO", "QRO", "MEX", "TLAX", and "PUE") and south east part of the country ("QROO and CHPS"). However, with the exception of Quintana Roo, none of these states presents the larger rate of suicides in the country. 
...
male_state_suicide_plot<- mxhexbin_choropleth(male_state_suicides, num_colors = 4) +
 labs(title = " Map 2. Average Age of Male Suicide Cases 1998-2017",
       subtitle = "Younger suicide cases concentrate in the center and southeast",
      legend = "Age",
...
In the case of female suicides, the states with the younger cases are similar as in the male case, except for Queretaro ("QRO") which is not among the top 25%, and instead the state of Guerrero ("GRO") and Chiapas ("CHPS") appear in this category. Map 3 shows this information. 

These two maps suggest that the states at the center and south east of the country concentrate most of the cases of young suicides, but not necessarily the higher rates (with the exception of Quintana Roo).
...
Chart 4 shows the distribution of suicide cases by occupation and age for the population between 25 and 50 years old. A special interest to show this chart is the analysis of how unemployment may be associated with suicide cases. 

The largest share of occupations in the suicide population across all ages are: 1) unemployed people, and those who are looking for a job (orange area on top); 2) Surveillance and personal services jobs; and 3) Workers in the manufacturing and transport industry. People in the sales business show an increasing share as people get older. Also, it could be fairly said that suicides are prevalent among the blue-collar worker population.
...
scale_fill_manual(values=ami_pal("multi")(12)) +
  ...
  labs(title = "4. Suicide Cases by Ocupation and Age",
...
The heatmaps below analyze the temporal pattern of suicides occurence. Time and day are an important characteristic of the suicides phenomenon, and an intervention aimed to prevent suicides should consider when suicides are more likely to occur. 

Chart 5 shows the time patterns for male and female suicide cases. Whereas for males the likelihood of suicide occurence is greater during the weekends (the horizontal blocks for Sunday and Monday), and in any day around midnight (the vertical line on zero), these patterns are still present in the female case, but with a more homogenous distribution during the weekdays. [^1]
...
heat_map_suicide_male <- ggplot(test_suicides %>% filter(!is.na(day_of_suicide) & !is.na(hour_suicide) & sex=='Male'), aes(hour_factor, day_of_suicide,  fill=n)) +  
 geom_tile(aes(fill = n), color="white") +
  ...
  ggtitle("5. Time Patterns in Suicides")+
  labs( subtitle = "Suicides are most likely to occur around midnight all weekdays,\n and during weekends.This pattern is more pronounced in males",
      caption="" ,x = "Time", y="") +
...
## 2.6 Chihuahua and Yucatan States Comparison

To analyze some of the characteristics of the states with the larger suicides rates, treemaps were created to show simmilarities and contrasts between the national characteristics of suicide population, and those of Yucatan and Chihuahua states. 

Regarding age and sex structure of suicide population (Figure 6A), both Chihuahua and Yucatan show a similar structure as the national one. However, in Chihuahua we observe more suicides in male older cohorts as compared to the whole country and Yucatan.



```{r yucatan and chihuahua, warning=FALSE, message=FALSE, error=FALSE, results='hide' }

library(treemapify)
library(treemap)
#age_years<15 ~ 'Less than 15',
suicides_yuc <-
  filter(death_certificates, non_natural_causes== 'Suicide' &age_years>=15)
suicides_yuc <-suicides_yuc %>%  mutate(age_cohort = case_when(
                                           age_years>=15 & age_years<35 ~ '15 to 35',
                                           age_years>=35 & age_years<50 ~ '35 to 50',
                                           age_years>=50 & age_years<70 ~ '50 to 70',
                                           age_years>=70 ~ 'More than 70'))
...
## Plots ####
palette_cohort_treemap<- c("#4C64E8", "#6190A6", "#93A15C", "#E19509")
treemap_cohorts<-ggplot(subset(cohort_suicides_yuc, sex!="Not specified" ), aes(area = n, subgroup=sex, fill=age_cohort, label = age_cohort)) +
  geom_treemap( alpha=0.8, palette=ami_pal("cool")(4)) + 
  geom_treemap_text( colour = "white", place = "centre",
                    grow = TRUE, family="Karla", min.size = 0, palette=ami_pal("cool")(4)) +
  ...
  labs(title = "6A. Suicides Cases in Yucatan and Chihuahua by Sex and Age",
        caption = "Source: INEGI") +
    scale_fill_manual(values=palette_cohort_treemap, aesthetics = "fill") +
  theme_ami 
treemap_cohorts

palette_occup_treemap<- c("#4C64E8", "#516FD7", "#577AC7", "#5C85B7", "#6190A6", "#679B96", "#6DA786", "#80A471", "#93A15C", "#A79E47","#BA9B32", "#CD981D", "#E19509")
treemap_occupation<-ggplot(subset(occup_suicides_yuc, occupation!="NA"), aes(area = n, fill=occupation, label = occupation)) +
  ...
  facet_wrap( ~state) + guides(fill=FALSE) +
    scale_fill_manual(values=palette_occup_treemap, aesthetics = "fill")
...
palette_school_treemap<-c("#4C64E8", "#597EC0", "#669999", "#84A36C", "#B29C3A", "#E19509")
treemap_school<-ggplot(subset(educ_suicides_yuc, school_category!="NA"), aes(area = n, fill=school_category, label = school_category)) +
  ...
  geom_treemap_text( colour = "white", place = "centre",
    ...
    theme(legend.position="bottom") + guides(fill=FALSE)+
    scale_fill_manual(values=palette_school_treemap, aesthetics = "fill")+
  labs(title = "6C. Suicides Cases in Yucatan and Chihuahua by School Category",
       subtitle="Population older than 25",
       caption = "Source: INEGI") +
theme_amiplots


palette_marital_treemap<-c("#4C64E8", "#5C85B7", "#6DA786", "#A79E47", "#E19509")
treemap_marital<-ggplot(subset(marital_suicides_yuc, marital_status!="NA"), aes(area = n, fill=marital_status, label = marital_status)) +
  ...
  geom_treemap_text( colour = "white", place = "centre",
     ...
     guides(fill=FALSE)+
   scale_fill_manual(values=palette_marital_treemap, aesthetics = "fill")+
...
Figure 6B shows the breakdown by occupation. The first  thing to note is the size of unemployed people share, which is the largest in the three cases. Interestingly, Chihuahua, a state which economy relies heavily on the manufacturing industry (and the share of the employed population in this sector is among the top in the country), shows a smaller share of this category as compared to the national level, which suggests that the distribution of occupations among suicide cases does not necessarily follow the distribution of occupations in a geographic unit, and some employment regional characteristics maybe important to characterize this population. In the state of Yucatan, the share of administrative workers is larger than the national level and Chihuahua. The same situation occurs for the Artisan/Auxiliary workers category in the state of Chihuahua. 
...
We also analyze the schooling degree of suicide population as a proxy of economic status. To avoid a bias of young suicide population that did not complete highschool or college, we subset the data to the population older than 25 years old. When we take a look at the school levels of suicide population, we observe at the national level that a majority has, at most, only complete elementary school, and only a minority of the suicide population completed a professional degree (licenciatura), which corresponds to the upper-right orange rectangle. As in the case of homicides, suicides affect relatively more young, male, unschooled population.


```{r treemap school, echo=FALSE, message=FALSE, warning=FALSE }
treemap_school
```
